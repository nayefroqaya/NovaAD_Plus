...
class AnomalyDetector:
    ...
    def anomaly_detector(df_full_train_labeled_features, sequences_df):
        ...
        # --------------------------
        # 2) Train deterministic XGB (reduce internal randomness)
        # --------------------------
...
#
        ...
        for off in offset_grid:
            ...
            rg = recall_score(y_val, gated_preds, pos_label=1, zero_division=0)
            f1g = f1_score(y_val, gated_preds, pos_label=1, zero_division=0)

            if rg >= TARGET_RECALL and f1g > best_f1_gate:
                best_f1_gate, best_offset = f1g, float(off)
        ...
        # --------------------------case2 --------------------------------------------------------------------------
        # ------- classification stage. XGBClassifier-----New ------------------------------------------------------
        # ======================================
    ...
    def anomaly_detector(df_full_train_labeled_features, sequences_df):
        ...
        df_real_normal = sequences_df.filter(col("Temp_label") == 0).select(col("Node_block_id"),
            col("features_vec_final"), col("anomaly_score_pca"), col("anomaly_score_gmm"), col("pca_flag"),
            col("gmm_flag"), lit(0).alias("Final_Label"), lit("real").alias("src"))

        df_pseudo_all = df_full_train_labeled_features.select(col("Node_block_id"), col("features_vec_final"),
            col("anomaly_score_pca"), col("anomaly_score_gmm"), col("pca_flag"), col("gmm_flag"),
        ...
        print(f"[INFO] Train counts n0={n0}, n1={n1}, "
        ...
        # --------------------------
        test_df = sequences_df.filter(col("Temp_label") == 888).select(col("Node_block_id"), col("features_vec_final"),
            col("y_true").alias("Final_Label"), col("anomaly_score_pca"), col("anomaly_score_gmm"), col("pca_flag"),
            col("gmm_flag")).withColumn("Final_Label", col("Final_Label").cast("int"))
        ...
        val_pdf = val_pred.select(col("Final_Label").alias("y"),
            vector_to_array(col("probability")).getItem(1).alias("prob_1"), col("pca_flag"), col("gmm_flag")).toPandas()
        ...
        target_recall_grid = [0.25, 0.30,0.35,0.40,0.45,0.50,0.55,0.57,0.60, 0.80, 0.90, 0.95]
        ...
        if best_prec < 0:
            ...
            print(f"[INFO] Threshold by best class-1 F1: "
                  f"t={best_threshold:.3f}, F1={best_f1:.4f}")

        else:
            print(f"[INFO] Threshold by precision@recall>= {TARGET_RECALL}: "
                  f"t={best_threshold:.3f}, precision={best_prec:.4f}")

        if (p_val >= best_threshold).sum() == 0:
            best_threshold = float(np.quantile(p_val, 0.90))
            print(f"[WARN] Threshold produced 0 anomalies on VAL. "
        ...
        print(f"[INFO] best_threshold={best_threshold:.4f}, "
              f"best_offset={best_offset:.3f}, gate_t={gate_t:.4f}")
        ...
        df_train_normal = sequences_df.filter(col("Temp_label") == 0).select(col("Node_block_id"),
            col("features_vec_final"), col("anomaly_score_pca"), col("anomaly_score_gmm"), col("pca_flag"),
            col("gmm_flag"), lit(0).alias("Final_Label"))

        df_pseudo_anomalies = df_full_train_labeled_features.filter(col("Final_Label") == 1).select(
            col("Node_block_id"), col("features_vec_final"), col("anomaly_score_pca"), col("anomaly_score_gmm"),
            col("pca_flag"), col("gmm_flag"), col("Final_Label").cast("int").alias("Final_Label"))
        ...
        # ======================================
        gbt_case2 = SparkXGBClassifier(features_col=features_col, label_col="Final_Label", max_depth=4, eta=0.05,
            n_estimators=500, subsample=0.85, colsample_bytree=0.80, scale_pos_weight=1.5, num_workers=2,
            eval_metric="logloss", seed=42)
        ...
        print(f"final Model classification completed in "
        ...
        print(f"final Model predicts completed in "
              f"{case2_Classification_pred_time:.2f} minutes")

        case2_test_pdf = test_pred.select(col("Final_Label").alias("y"),
            vector_to_array(col("probability")).getItem(1).alias("prob_1"), col("pca_flag"), col("gmm_flag")).toPandas()

        case2_test_pdf["pred_gbt"] = (case2_test_pdf["prob_1"] >= best_threshold_case2).astype(int)
        ...
        return (best_target_recall, model_case1, model_case2, case1_test_pdf, case2_test_pdf, case1_classification_time,
